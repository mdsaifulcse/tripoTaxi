...
class SendPushNotification extends Controller
{
    ...
    /**
     * Payment Done with ssl.
     *
     * @return void
     */
    public function SSLPayment($user_id, $money){
        $user = User::where('id',$user_id)->first();
        $language = $user->language;
        App::setLocale($language);
        return $this->sendPushToUser($user_id, $money.' '.trans('api.push.payment_with_ssl'));
    }

    public function ProviderSSLPayment($user_id, $money){
        $user = Provider::where('id',$user_id)->first();
        $language = $user->language;
        App::setLocale($language);

        return $this->sendPushToProvider($user_id, $money.' '.trans('api.push.payment_with_ssl'));
    }
