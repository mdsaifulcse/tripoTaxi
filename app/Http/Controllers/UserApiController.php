...
//        $token = getenv("TWILIO_AUTH_TOKEN");
        ...
        try{
            ...
            if(!empty($userData))
            {
                return response()->json(['message' =>"You have already registered "], Response::HTTP_CONFLICT);
            }
            ...
            return response()->json(['message' =>"New OTP : $newOtp and Validity: $otpValidity"], Response::HTTP_CREATED);
    ...
    public function verifyOtp(Request $request)
    {
        ...
        try{
            ...
            if (empty($userOtpData))
            {
                return response()->json( ['message'=>'Your given otp does not match'],Response::HTTP_NO_CONTENT);
            }
            ...
            if ($userOtpData->status==UserRegOtp::NOT_VERIFIED && $validData->gt(Carbon::now()))
            {
                ...
                return response()->json(['message'=>'Given Otp Verified '.$userOtpData->phone],Response::HTTP_OK);

            }else{

                return response()->json(['message'=>'Your given otp has been expired '],Response::HTTP_UNAUTHORIZED);
            }
	...
	public function login(Request $request)
	{
	    ...
	    if(!User::where(['mobile'=>$request->username, 'country_code'=>$request->country_code])->first()) {
	        return \Response::json(["error" => "The Mobile field is incorrect.","message" => "The Mobile field is incorrect."], 401);
	    }
		...
		if(empty($json['error'])){
			if(Auth::guard("web")->attempt(['mobile'=>$request->username, 'country_code'=>$request->country_code, 'password' => $request->password])) {
				$user = Auth::guard("web")->user();
				if($user->status == 'banned') {
					return \Response::json(["error" => "Opps! Your Account is banned.","message" => "Opps! Your Account is banned."], 401);
				}
				if($user) {
	...
	public function details(Request $request)
	{
		...
		try{
			...
			if($user = User::find(Auth::user()->id)){
	            ...
	            $user->ssl =(int)config('constants.ssl');
	...
	public function send_request(Request $request) 
	{
		...
		if($request->ajax()) {
			$this->validate($request, [
				...
				'use_wallet' => 'numeric',
				'payment_mode' => 'required|in:BRAINTREE,SSL,CASH,CARD,PAYPAL,PAYPAL-ADAPTIVE,PAYUMONEY,PAYTM',
		...
		try{
		    ...
		    //Start of parcelmate logic
			$UserRequest->name =  $request->r_fullname ? : "";
            $UserRequest->phone = $request->r_phonenumbe_r ? : "";
            $UserRequest->email = $request->r_email ? : "";
            $UserRequest->address = $request->r_address ? : "";
            $UserRequest->document_type = $request->p_type ? : "";
            $UserRequest->others = $request->p_note ? : "";
            //End of parcelmate logic
...
// 			}
    ...
    public function verifyPhone(Request $request)
	{
		...
		if($request->has("mobile")) {
			...
			if($Provider != null) {
				return response()->json(['isAvaiable' => false]);
			}else{
			    return response()->json(['isAvaiable' => true]);
			}
		}
...
// 			$Provider = User::where('email', $request->input("email"))->first();
// 			if($Provider != null) {
// 				return response()->json(['isAvaiable' => trans('api.email_exist')]);
// 			}
// 		}
