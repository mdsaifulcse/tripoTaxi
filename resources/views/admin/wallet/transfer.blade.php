        ...
        <div class="container-fluid">
            ...
            @if($type=='provider')
                <div class="box box-block bg-white">
                    ...
                    @endif
                    <h5 class="mb-1">Provider List </h5>

                    {{-- @if(Setting::get('demo_mode', 0) == 0)
                        <a href="{{route('admin.transfercreate', $flag)}}" style="margin-left: 1em;" class="btn btn-primary pull-right"><i class="fa fa-plus"></i> @lang('admin.addsettle')</a>
                    @endif --}}
                    <table class="pptbl table table-striped table-bordered">
                        <thead>
                            <tr>
                                <th>@lang('#')</th>
				...
				<th>Total Paid</th>
                                <th>Last Payment @lang('admin.datetime')</th>
                        ...
                        <tbody>
                            @foreach($totalList as $key => $pro)
                            <tr>
                                <td>{{$key +1}}</td>
                                <td>{{ $pro->first_name." ".$pro->last_name }}</td>                            
                                <td>{{ $pro->email }}</td>                            
                                <td>{{ $pro->country_code . $pro->mobile }}</td>
				<td>{{ currency($pro->wallet_requests->where('status', '=', 1)->sum('amount')) }} </td> 
                                <td>{{ $pro->wallet_requests->where('status', '=', 1)->last()->updated_at != NULL ? $pro->wallet_requests->where('status', '=', 1)->last()->updated_at->diffForHumans() : '' }}</td>
                                <td>{{ currency($pro->wallet_requests->where('status', '=', 1)->last()->amount) }} </td>
                                <td>{{ currency($pro->wallet_balance) }}</td>
                                {{-- <td> 
                                    @if( Setting::get('demo_mode', 0) == 0)                  
                                        <!-- <a class="btn btn-success btn-block" href="{{ route('admin.approve', $pending->id) }}">@lang('admin.approve')</a> -->
                                        <button type="button" class="btn btn-success btn-block transferClass" data-toggle="modal" data-target="#transferModal" data-id="send" data-href="{{route('admin.approve', $pending->id) }}" data-rid="{{$pending->id}}">@lang('admin.approve')</button>
